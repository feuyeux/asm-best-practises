...
kubectl \
  ...
  delete namespace migrant-hello >/dev/null 2>&1
...
kubectl \
  --kubeconfig "$USER_CONFIG" \
  create ns migrant-hello

kubectl \
  --kubeconfig "$USER_CONFIG" \
  label ns migrant-hello istio-injection=enabled
...
echo "waiting for hello-workload"
kubectl \
  --kubeconfig "$USER_CONFIG" \
  -n migrant-hello \
  wait --for=condition=ready pod -l app=hello-workload
