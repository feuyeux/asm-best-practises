...
for vm in "${VMS[@]}"; do
  k exec "$hello1_pod" -c hello-v1-deploy -n grpc-reciprocal-hello \
    -- grpcurl -plaintext -d '{"name":"eric"}' "$vm":7001 org.feuyeux.grpc.Greeter/SayHello
  echo
done
echo
echo "Test access hello2-svc"
k exec "$hello1_pod" -c hello-v1-deploy -n grpc-reciprocal-hello -- grpcurl -plaintext -d '{"name":"eric"}' \
  hello2-svc.grpc-reciprocal-hello.svc.cluster.local:7001 org.feuyeux.grpc.Greeter/SayHello
echo
echo
echo "Test access hello1-svc"
k exec "$hello1_pod" -c hello-v1-deploy -n grpc-reciprocal-hello -- grpcurl -plaintext -d '{"name":"eric"}' \
  hello2-svc.grpc-reciprocal-hello.svc.cluster.local:7004 org.feuyeux.grpc.Greeter/SayHello
echo
