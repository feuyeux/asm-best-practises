...
echo "== 1 Test access =="
k exec -it "$FORTIO_POD" -c fortio -n http-circuit-breaking -- fortio load -curl http://hello2-svc:8001/hello/eric
echo
...
result_c2_n20=$(k exec -it "$FORTIO_POD" -c fortio -n http-circuit-breaking -- \
  fortio load -c 2 -qps 0 -n 20 -loglevel Warning http://hello2-svc:8001/hello/eric)
echo "$result_c2_n20"
echo
...
result_c3_n30=$(k exec -it "$FORTIO_POD" -c fortio -n http-circuit-breaking -- \
  fortio load -c 3 -qps 0 -n 30 -loglevel Warning http://hello2-svc:8001/hello/eric)
echo "$result_c3_n30"
echo
echo "circuit breaking status:"
